...
void Lvgl_PortInit(DisplayPort &display) {
	...
	// Draw buffers must be internal DMA RAM: the QSPI panel flush is then
	// zero-copy. PSRAM buffers force spi_master to stage through an internal
	// "priv TX buffer", which fails to allocate and whites out the panel
	// (logs/clare_s3_boot2_20260829.log). Internal RAM is freed up by
	// LV_USE_CLIB_MALLOC + WIFI_IRAM_OPT=n (see sdkconfig.defaults).
